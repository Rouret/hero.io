...
export default class ShooterServer{
  app: express.Application;
    ...
    _loopPlayers() {
      this.game.players.forEach((player) => {
        ...
        if (boostCollided !== null) {
          ...
          this.game.boosts = this.game.boosts.filter((b) => b.id !== boostCollided.id);
        }
      });
    }
    ...
    _startGameLoop() {
      setInterval(() => {
        this.taskLoop.forEach((task) => task());
        this.io.emit("update", { players: this.game.players, bullets: this.game.bullets, boosts: this.game.boosts });
      }, 1000 / this.tickrate);
    }
